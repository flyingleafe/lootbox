...
data ConfigKind
  = Partial
  -- ^ In a partial configuration some of the fields might be missing, in case
  -- they will be initialised later.
  | Final
  -- ^ A final configuration must have all its fields initialised.
...
finalise'
  ...
  -- ^ Option name prefix
  -> Maybe String
  -- ^ Name of a branch we are looking for (if any)
...
option
  :: forall k l v g is a. (Functor g, a ~ Item' k (l ::: v), HasOption l is v)
  => Label l
  -> (a -> g a)
  -> ConfigRec k is
  -> g (ConfigRec k is)
...
sub
  :: forall k l us g is a. (Functor g, a ~ Item' k (l ::< us), HasSub l is us)
  => Label l
  -> (a -> g a)
  -> ConfigRec k is
  -> g (ConfigRec k is)
...
tree
  :: forall k l us g is a. (Functor g, a ~ Item' k (l ::+ us), HasSum l is us)
  => Label l
  -> (a -> g a)
  -> ConfigRec k is
  -> g (ConfigRec k is)
...
branch
  :: forall k l us g is a. (Functor g, a ~ Item' k (l ::- us), HasBranch l is us)
  => Label l
  -> (a -> g a)
  -> ConfigRec k is
  -> g (ConfigRec k is)
...
instance
  ...
  ) => Monoid (Item 'Partial (l ::< is)) where
