...
import Loot.Config.Record (ConfigKind (..), ConfigRec, HasOption, HasSub,
                           ItemKind, option, sub, type (::+), type (::-),
                           type (:::), type (::<))
...
type family DoesntHaveType f is :: Constraint where
  DoesntHaveType _ '[]                = ()
  DoesntHaveType f ((_ ::: f)  ': is) =
    TypeError ('Text "DoesntHaveType failed (item type not unique?)")
...
type family LabelOfType (f :: Type) labels cont (is :: [ItemKind]) :: [SymPathElem] where
  ...
  LabelOfType f labels _ ((l ::: f) ': _) = 'SOption l ': labels
  LabelOfType f labels cont ((l ::: g) ': is) = LabelOfType f labels cont is
...
instance (HasOption l is v) => HierarchyLens ('SOption l ': lx) is v where
  hlens = option (Label :: Label l)

instance
  (HasSub l is us, HierarchyLens lx us v)
  => HierarchyLens ('SSub l ': lx) is v where
  hlens = sub (Label :: Label l) . (hlens @lx :: Lens' (ConfigRec 'Final us) v)
